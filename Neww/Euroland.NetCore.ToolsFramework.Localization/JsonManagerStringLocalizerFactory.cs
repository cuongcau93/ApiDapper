...
namespace Euroland.NetCore.ToolsFramework.Localization
{
    public class JsonManagerStringLocalizerFactory : IStringLocalizerFactory
    {
        ...
        /// <summary>
        /// Gets the <see cref="ILanguageToCultureProvider"/>
        /// </summary>
        public ILanguageToCultureProvider LanguageToCultureProvider
        {
            get { return this._languageToCultureProvider; }
        }
            ...
            IOptions<JsonLocalizationOptions> localizationOptions)
        {
            ...
            this._languageToCultureProvider = options?.LanguageToCultureProvider;
            this._loggerFactory = loggerFactory;

            if (this._languageToCultureProvider == null)
            {
                this._languageToCultureProvider = this.CreateLanguageToCultureOrDefault();
            }

            if (!this._languageToCultureProvider.Loaded)
            {
                this._languageToCultureProvider.Load();
            }
        }
        ...
        private ILanguageToCultureProvider CreateLanguageToCultureOrDefault()
        {
            ...
            {
                System.IO.FileInfo physicalFileInfo = null;
                
                if (System.IO.Path.IsPathRooted(_languageToCultureResourcePath))
                {
                    physicalFileInfo = new System.IO.FileInfo(_languageToCultureResourcePath);
                }
                else
                {
                    physicalFileInfo = new System.IO.FileInfo(System.IO.Path.Combine(_appHostingEnvironment.ContentRootPath, _languageToCultureResourcePath));
                }
                    ...
                    && physicalFileInfo.Exists)
                {
                    ...
                    var fileProvider = new Microsoft.Extensions.FileProviders.PhysicalFileProvider(physicalFileInfo.DirectoryName);
                    Microsoft.Extensions.Primitives.ChangeToken.OnChange(
                        () => fileProvider.Watch(physicalFileInfo.Name),
                        () => LanguageToCultureProvider.Load(reload: true)
                    );
                }
            }
        ...
        private JsonManagerStringLocalizer CreateJsonManagerStringLocalizer(string baseName)
        {
            ...
            return this._localizerCache.GetOrAdd(baseName, (key) => new JsonManagerStringLocalizer(
                ...
                this.LanguageToCultureProvider)
            );
        }

...
namespace Euroland.NetCore.ToolsFramework.Test.Setting
{
    public class RequestSettingTest
    {
        ...
        public void CanHandleSettingFromRequestQueryString()
        {
            ...
            {
                new QueryStringRequestSettingFinder(HttpUtils.MockHttpContext())
            };
            
            var rootPath = Path.Combine(TestUtils.CurrentRootDirectory, @"tools\ToolTest\config\company");
            var requestSettingProvider = new RequestSettingProviderFactory(rootPath, supportedRequestSettingFinders);
        ...
        public void CanHandleSettingFromFormCollection()
        {
            ...
            var supportedRequestSettingFinders = new IRequestSettingFinder[]
            {
                new FormRequestSettingFinder(HttpUtils.MockHttpContext())
            };
        ...
        public void ImplicitReplacement_and_Merge_4_Levels_setting()
        {
            ...
            settingFactory.Accept(new RequestSettingProviderFactory(companySettingRootPath, supportedRequestSettingFinders) { ResourceType = SettingResourceType.Json_Xml });
            settingFactory.Accept(new RequestSettingProviderFactory(toolCompanySettingRootPath, supportedRequestSettingFinders) { ResourceType = SettingResourceType.Json_Xml });
